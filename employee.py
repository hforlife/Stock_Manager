...
# title
        title = Label(self.root, text="Information de l'employé", font=("Lato", 14, "normal"), bg="#F66B0E", fg="white")
        title.place(x=50, y=20, width=1000)
        # content
        # -----first row-----
        emp_id_label = Label(self.root, text="Emp ID", font=("Lato", 14, "normal"), bg="lightgray")
        emp_id_label.place(x=50, y=70)
        gender_label = Label(self.root, text="Sexe", font=("Lato", 14, "normal"), bg="lightgray")
        gender_label.place(x=350, y=70)
        contact_label = Label(self.root, text="Contact", font=("Lato", 14, "normal"), bg="lightgray")
        ...
        gender_opt = ttk.Combobox(self.root, textvariable=self.gender_var, values=("Select", "Homme", "Femme"),
                                  state="readonly", justify=CENTER, font=("Lato", 14, "normal"))
        gender_opt.place(x=500, y=70, width=180)
        gender_opt.current(0)
        contact_txt = Entry(self.root, textvariable=self.contact_var, font=("Lato", 14, "normal"), bd=1,
                            bg="#EEE6CE")
        ...
        dob_label = Label(self.root, text="Date Naiss", font=("Lato", 14, "normal"), bg="lightgray")
        dob_label.place(x=350, y=110)
        doj_label = Label(self.root, text="Date Adh", font=("Lato", 14, "normal"), bg="lightgray")
        ...
        # -----third row-----
        email_label = Label(self.root, text="Email", font=("Lato", 14, "normal"), bg="lightgray")
        email_label.place(x=50, y=150)
        password_label = Label(self.root, text="Password", font=("Lato", 14, "normal"), bg="lightgray")
        password_label.place(x=350, y=150)
        user_type_label = Label(self.root, text="Type", font=("Lato", 14, "normal"), bg="lightgray")
        ...
        password_txt = Entry(self.root, textvariable=self.password_var, font=("Lato", 14, "normal"),
                             bg="#EEE6CE", bd=1)
        password_txt.place(x=500, y=150, width=180)
        user_type_opt = ttk.Combobox(self.root, textvariable=self.usertype_var, values=("Admin", "Employé"),
                                     state="readonly", justify=CENTER, font=("Lato", 14, "normal"))
        user_type_opt.place(x=850, y=150, width=180)
        user_type_opt.current(0)

        # -----fourth row-----
        address_label = Label(self.root, text="Adresse", font=("Lato", 14, "normal"), bg="lightgray")
        address_label.place(x=50, y=190)
        salary_label = Label(self.root, text="Salaire", font=("Lato", 14, "normal"), bg="lightgray")
        ...
        # buttons
        add_btn = Button(self.root, text="Ajouter", command=self.add_emp, font=("Lato", 11, "bold"), bg="#2EB086",
                         fg="white", bd=3, cursor="hand2")
        add_btn.place(x=500, y=225, width=110, height=25)
        update_btn = Button(self.root, text="Modifier", command=self.update_emp, font=("Lato", 11, "bold"),
                            bg="#0AA1DD", fg="white", bd=3, cursor="hand2")
        update_btn.place(x=620, y=225, width=110, height=25)
        delete_btn = Button(self.root, text="Supprimer", command=self.delete_emp, font=("Lato", 11, "bold"),
                            bg="#B8405E", fg="white", bd=3, cursor="hand2")
        delete_btn.place(x=740, y=225, width=110, height=25)
        clear_btn = Button(self.root, text="Effacer", command=self.clear, font=("Lato", 11, "bold"),
                           bg="#313552", fg="white", bd=3, cursor="hand2")
        clear_btn.place(x=860, y=225, width=110, height=25)

        # search employee
        search_frame = LabelFrame(self.root, text="Chercher un Employé", font=("Lato", 11, "normal"), bg="lightgray", bd=2)
        search_frame.place(x=250, y=260, width=600, height=100)

        # search options
        options_box = ttk.Combobox(search_frame, textvariable=self.searchOption_var,
                                   values=("Selectionner", "Email", "Nom", "Contact"), state="readonly", justify=CENTER,
                                   font=("Lato", 11, "normal"))
        ...
        search_btn = Button(search_frame, text="Chercher", command=self.search_emp, font=("Lato", 11, "bold"),
                            bg="#2EB086", fg="white", bd=3, cursor="hand2")
        ...
        list_columns = ("id", "nom", "email", "sexe", "contact", "date.naiss", "date.adh", "password", "type",
                        "adresse", "salaire")
        self.emp_list_table = ttk.Treeview(emp_list_frame, columns=list_columns, yscrollcommand=scroll_y.set,
        ...
        try:
            if self.emp_id_var.get() == "":
                ...
                if row is not None:
                    ...
                    cur.execute("INSERT INTO employee (id, name, email, gender, contact, dob, doj, password, "
                                "type, address, salary) VALUES (?,?,?,?,?,?,?,?,?,?,?)", values_to_insert)
                    con.commit()
    ...
    def update_emp(self):
        ...
        try:
            if self.emp_id_var.get() == "":
                messagebox.showerror("Erreur: ", "Vous devez saisir l'ID de l'employé", parent=self.root)
            else:
                cur.execute("SELECT * FROM employee WHERE id=?", (self.emp_id_var.get(),))
                row = cur.fetchone()
                if row is None:
                    messagebox.showerror("Erreur: ", "ID de l'Employé Invalide", parent=self.root)
                else:
                    values_to_insert = (
                                        self.name_var.get(),
                                        self.email_var.get(),
                    ...
                    cur.execute("UPDATE employee set name=?, email=?, gender=?, contact=?, dob=?, doj=?, "
                                "password=?, type=?, address=?, salary=? where id=?", values_to_insert)
                    con.commit()
                    messagebox.showinfo("Succès: ", "Info de l'employé modifié avec succès", parent=self.root)
                    self.show_emp()
                    con.close()
        except Exception as ex:
            messagebox.showerror("Erreur: ", f"Erreur: {str(ex)}", parent=self.root)
        ...
        try:
            if self.emp_id_var.get() == "":
                ...
                if row is None:
                    ...
                    user_confirm = messagebox.askyesno("Confirmation:", "Confirmez-vous la suppression?", parent=self.root)
                    if user_confirm:
                        cur.execute("DELETE FROM employee WHERE id=?", (self.emp_id_var.get(),))
                        con.commit()
                        messagebox.showinfo("Succès: ", "Employé supprimer avec succès", parent=self.root)
                        self.show_emp()
                        self.clear()
    ...
    def search_emp(self):
        ...
        try:
            if self.searchOption_var.get() == "Selectionner":
                ...
                cur.execute("SELECT * FROM employee WHERE " + self.searchOption_var.get() +
                            " LIKE '%" + self.searchText_var.get() + "%'")
                rows = cur.fetchall()
                if len(rows) != 0 :
                    self.emp_list_table.delete(*self.emp_list_table.get_children())
                    for row in rows:
